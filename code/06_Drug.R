...
top_anno_pRRophetic <- build_top_anno(cor_r_pRRophetic, cor_p_pRRophetic,
                                 colnames(cor_r_pRRophetic), show_legend = TRUE)
...
rng <- range(cor_r_pRRophetic, na.rm = TRUE)
col_fun <- colorRamp2(seq(-max(abs(rng)), max(abs(rng)), length.out = 11),
                      rev(brewer.pal(11, "RdBu")))

cn_pRRophetic <- colnames(cor_r_pRRophetic)

bottom_anno_pRRophetic <- HeatmapAnnotation(
  text = anno_text(cn_pRRophetic, rot = 60, location = unit(1, "npc"), just = "right"),
  annotation_height = max_text_width(cn_pRRophetic)
)

ht_pRRophetic <- Heatmap(
  cor_r_pRRophetic,
  name                = "Rs",
  col                 = col_fun,
  column_title        = "pRRophetic",
  row_names_gp        = gpar(fontsize = 9),
  show_row_names      = TRUE,
  show_column_names   = FALSE,
  cluster_rows        = TRUE,
  cluster_columns     = TRUE,
  top_annotation      = top_anno_pRRophetic,
  bottom_annotation   = bottom_anno_pRRophetic,
  right_annotation    = right_anno,
  cell_fun            = make_cell_fun(cor_p_pRRophetic),
  show_heatmap_legend = TRUE
)

pdf(file.path(DIR_FIG, "Heatmap_pRRophetic.pdf"), width = 6, height = 8)
draw(ht_pRRophetic,
     merge_legends          = TRUE,
...
sig_pos_drug <- apply(cor_r_pRRophetic > 0.2 & cor_p_pRRophetic < 0.05, 1,
                      function(x) any(x, na.rm = TRUE))
sig_neg_drug <- apply(cor_r_pRRophetic < -0.2 & cor_p_pRRophetic < 0.05, 1,
                      function(x) any(x, na.rm = TRUE))

drug_sig_df <- data.frame(
  Drug    = rownames(cor_r_pRRophetic),
  Pathway = pathway_vec,
  Pos     = sig_pos_drug,
  Neg     = sig_neg_drug,
